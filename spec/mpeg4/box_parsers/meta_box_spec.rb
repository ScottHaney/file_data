...
RSpec.describe FileData::MetaBoxParser do
  ...
  describe '#parse' do
    context 'when there is an apple creation date key/value pair' do
    ...
    context 'when there is not an apple creation date key but no matching value' do
      let(:bytes) do
        apple_key = 'com.apple.quicktime.creationdate'.each_byte.map { |x| x }
        custom_str = 'testing'
        ...
        [[0, 0, 0, 24 + apple_key.length], # keys box size
         ...
         [0, 0, 0, 2], # index that does not match
    ...
    context 'when there are no key values but ilst values are present' do
      let(:bytes) do
        custom_str = 'testing'
        custom_bytes = custom_str.each_byte.map { |x| x }
    ...
    context 'when there are no ilst values but key values are present' do
      let(:bytes) do
        ...
        [[0, 0, 0, 24 + custom_key.length], # keys box size
         ...
         [0, 0, 0, 0], # key1 namespace
         custom_key].flatten # key1 value of apple_key
      end
